...
def load_data():
    ...
    if use_SQL:
        ...
        print('Loading historical data from OTM...')

        # Connect to OTM database
...
def abt(org_df, dimension):
    """Transform the original data set into ABT table by specific dimension provided in 2nd argument. By customer, then input string "DestName" in dimension. By region, then input string "DestZone2"
...
def calc_imt(df_imt_abt):
    ...
    print('IMT Volume Table:')
    print(imt_table_df)
    # Render table of IMT volumes (should be user-editable later)
    imt_vol_table = render_t_table(imt_table_df,
                                   'imt-vol-table',
                                   'Date',
                                   '%b %d')
    ...
    return imt_vol_div, imt_frf_div
...
def calc_dt(df_dt_abt):
    ...
    print('DT Volume Table:')
    print(dt_table_df)
    # Render table of DT volumes (should be user-editable later)
    dt_vol_table = render_t_table(dt_table_df,
                                  'dt-vol-table',
                                  'Date',
                                  '%b %d')
